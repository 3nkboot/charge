...
def kefara(ccx):
    try:
        ...
        r.post('https://www.northidahowaterpolo.org/wp-admin/admin-ajax.php', headers=headers, data=data, timeout=15)
        ...
        response = r.post('https://www.northidahowaterpolo.org/wp-admin/admin-ajax.php', params=params, headers=headers, data=multipart_data, timeout=15)
        ...
        r.post(f'https://cors.api.paypal.com/v2/checkout/orders/{tok}/confirm-payment-source', headers=headers_paypal, json=json_data, timeout=15)
        ...
        response = r.post('https://www.northidahowaterpolo.org/wp-admin/admin-ajax.php', params=params_approve, headers=headers, data=multipart_approve, timeout=15)
        ...
        if 'true' in text or 'sucsess' in text: return "✅ Charge !"
            ...
            try:
                err = response.json().get('data', {}).get('error', 'UNKNOWN_ERROR')
                return f"❌ {err}"
            except:
...
def handle_docs(message):
    try:
        if message.document.file_name.endswith('.txt'):
            ...
            if len(output) > 4000:
                with open("results_final.txt", "w") as f: f.write(output)
                with open("results_final.txt", "rb") as f:
...
if __name__ == "__main__":
    print("Bot is starting with bug fixes...")
